...
function spawnFood() {
    return { x: Math.random()*MAP_SIZE, y: Math.random()*MAP_SIZE, color: `hsl(${Math.random()*360}, 100%, 50%)`, size: 8 };
}

for(let i=0; i<300; i++) food.push(spawnFood());
for(let i=0; i<10; i++) viruses.push({ x: Math.random()*(MAP_SIZE-200)+100, y: Math.random()*(MAP_SIZE-200)+100, size: 65 });
...
io.on('connection', (socket) => {
    ...
    socket.on('split', () => {
        ...
        p.cells.forEach(cell => {
            if (cell.size >= 55 && p.cells.length + newCells.length < 16) {
                cell.size /= 1.41;
                newCells.push({
                    x: cell.x, y: cell.y, size: cell.size,
                    boostX: Math.cos(p.angle) * 25, boostY: Math.sin(p.angle) * 25,
                    lastSplit: Date.now()
                });
            }
        });
        ...
        p.cells.forEach(cell => {
            if (cell.size > 40) {
                ...
                food.push({ 
                    ...
                    color: p.color, size: 14, isEjected: true, vX: Math.cos(p.angle)*15, vY: Math.sin(p.angle)*15 
                });
            }
        });
    });
...
setInterval(() => {
    for (let id in players) {
        ...
        p.cells.forEach((cell, idx) => {
            let speed = (4.2 * (30 / cell.size) + 0.8);
            ...
            food.forEach((f, fIdx) => {
                if (Math.hypot(cell.x - f.x, cell.y - f.y) < cell.size) {
                    cell.size += f.isEjected ? 1.5 : 0.5;
                    food.splice(fIdx, 1);
                    if(!f.isEjected) food.push(spawnFood());
                }
            });
            ...
            for(let j = idx + 1; j < p.cells.length; j++){
                ...
                let canMerge = Date.now() - cell.lastSplit > 10000;
                if (dist < (cell.size + other.size) * (canMerge ? 0.3 : 0.8)) {
                    if (canMerge) {
                        cell.size = Math.sqrt(cell.size**2 + other.size**2);
                        p.cells.splice(j, 1);
                    } else {
                        let a = Math.atan2(cell.y - other.y, cell.x - other.x);
                        other.x -= Math.cos(a) * 2; other.y -= Math.sin(a) * 2;
                    }
                }
            }
        });
...
http.listen(3000, () => { console.log('Ready!'); });
